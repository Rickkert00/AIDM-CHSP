...
def emptyTime(tanks, emptys, i, j):
    totalTime = 0
    start = min(i, j)
    stop = max(i, j)
    for k in range(start, stop):
        if k - i <= tanks / 2 and k - j <= tanks / 2:
            totalTime += emptys[k]
        else:
            totalTime -= emptys[k]
        if totalTime <= 0:
            totalTime = int(random.uniform(1, 5))
    return totalTime
                    ...
                    for timeMinVariability in variability:
                        for emptyTimeRange in timeRange:
                            for loadedTimeVariability in variability:
                                ...
                                inst += 1
                                filename = "../instances/ring/" + str(inst)
                                filename += "."
                                filename += "dzn"
                                with open(filename, 'w') as f:
                                    ...
                                    for i in range(1, tanks + 1):
                                        for j in range(0, tanks + 1):
                                            f.write(str(emptyTime(tanks, emptys, i, j)))
                                            if j < tanks:
                                                f.write(",")
                                        if i < tanks + 1:
                                            f.write(",\n")
